...
4. **图片旋转功能**：在包含 `<img class="ans-ued-img">` 的页面，自动在图片上方添加"旋转90度"按钮，方便查看横向图片或需要旋转的图片。
...
3. **在浏览器中加载插件**
   ...
   2. 在页面右上角找到并启用"开发者模式"(Edge在左边的扩展按钮里面)
   3. 点击"加载已解压的扩展程序"
...
2. 插件会自动生效：
   ...
   - 在包含图片的页面，图片上方会显示"旋转90度"按钮，点击即可旋转图片
...
### Q: 图片旋转按钮没有显示？
A: 请确保页面包含 `<img class="ans-ued-img">` 元素，并且页面已完全加载。

>>>>>>> 8561fa3 (Add image rotation feature and update documentation)
### Q: 如何更新插件？
